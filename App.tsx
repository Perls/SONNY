...
// Always-visible core components (eagerly loaded)
import GameMapLayer from './components/GameMapLayer';
import LocationInteractionLayer from './components/LocationInteractionLayer';
...
import ConsiglierePanel from './components/ConsiglierePanel';

// Heavy view/menu components (lazy loaded — only parsed when needed)
const CharacterSelectScreen = React.lazy(() => import('./components/CharacterSelectScreen'));
const CombatScreen = React.lazy(() => import('./components/CombatScreen'));
const CharacterCreation = React.lazy(() => import('./components/CharacterCreation'));
const CrewList = React.lazy(() => import('./components/CrewList'));
const InventoryScreen = React.lazy(() => import('./components/InventoryScreen'));
const ProfileWindow = React.lazy(() => import('./components/ProfileWindow'));
const HoldingsDrawer = React.lazy(() => import('./components/HoldingsDrawer'));
const CharacterInspectWindow = React.lazy(() => import('./components/CharacterInspectWindow'));
const DailyTimes = React.lazy(() => import('./components/DailyTimes'));
const TagsDrawer = React.lazy(() => import('./components/TagsDrawer'));
const SettingsWindow = React.lazy(() => import('./components/SettingsWindow'));
const FamilyScreen = React.lazy(() => import('./components/FamilyScreen'));
const CombatOperations = React.lazy(() => import('./components/CombatOperations'));
const IndustrialJobMinigame = React.lazy(() => import('./components/IndustrialJobMinigame'));
const MinigameLayer = React.lazy(() => import('./components/MinigameLayer'));
const JournalScreen = React.lazy(() => import('./components/JournalScreen'));
const PlayerHousing = React.lazy(() => import('./components/PlayerHousing'));
...
const App: React.FC = () => {
    ...
    if (engine.view === 'splash') {
        return <Suspense fallback={<div className="fixed inset-0 bg-slate-900 flex items-center justify-center text-white text-xl">Loading...</div>}><CharacterSelectScreen saves={engine.saves} onSelectCharacter={engine.handleSelectCharacter} onCreateNew={engine.handleCreateNew} onDeleteCharacter={engine.handleDeleteCharacter} /></Suspense>;
    }

    if (engine.view === 'combat' && engine.gameState) {
        return (
            <Suspense fallback={<div className="fixed inset-0 bg-slate-900 flex items-center justify-center text-white text-xl">Loading Combat...</div>}>
                <CombatScreen
    ...
    if (engine.view === 'create') {
        return (
            <Suspense fallback={<div className="fixed inset-0 bg-slate-900 flex items-center justify-center text-white text-xl">Loading...</div>}>
                <CharacterCreation
